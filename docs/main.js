...
function questionsUrl(bankId = '') {
  ...
  return `${API_BASE}/api/questions${params}`;
}
...
async function renderEditQuestions() {
      ...
      // 1. 從後端讀題庫套組與目前選定套組的題目
      await loadQuestionBanks();
      if (!questionBanks.length) {
        currentAdminQuestionBankId = 'default';
      }
      const selectedBank = questionBanks.find(bank => bank.bankId === currentAdminQuestionBankId)
        || questionBanks.find(bank => bank.isActive)
        || questionBanks[0];
      if (selectedBank) currentAdminQuestionBankId = selectedBank.bankId;

      const res = await fetch(questionsUrl(currentAdminQuestionBankId));
      const qs  = await res.json();
    
      // 2. 產生表格
      let html = `<h2 class="text-2xl mb-4">題庫管理</h2>
        <div class="mb-4 flex flex-row flex-wrap gap-3 items-center justify-center">
          <label for="questionBankSelect" class="text-white">目前管理：</label>
          <select id="questionBankSelect" class="bg-gray-800 text-white border-2 border-yellow-400 rounded p-2">
            ${questionBanks.map(bank => `
              <option value="${escapeHtml(bank.bankId)}" ${bank.bankId === currentAdminQuestionBankId ? 'selected' : ''}>
                ${escapeHtml(bank.name)}（${bank.count ?? 0} 題）${bank.isActive ? ' - 學生使用中' : ''}
              </option>
            `).join('')}
          </select>
      ...
      qs.forEach((q, i) => {
        html += `<tr>
          ...
          <td>${escapeHtml(q.options[q.answer])}</td>
          <td>
            <button data-id="${escapeHtml(q._id)}" class="editQ btn text-sm mr-2">編輯</button>
            <button data-id="${escapeHtml(q._id)}" class="delQ btn text-sm">刪除</button>
          </td>
        </tr>`;
      });
      html += `</tbody></table>`;
      ...
      if (bankSelect) {
        bankSelect.addEventListener('change', async e => {
          currentAdminQuestionBankId = e.target.value;
          await renderEditQuestions();
        });
      }
      ...
      document.getElementById('createQuestionBankBtn')
        .addEventListener('click', async () => {
          ...
          if (!createRes.ok) return alert(created.message || '新增題庫失敗');
          currentAdminQuestionBankId = created.bankId;
          await renderEditQuestions();
        });
      ...
      document.getElementById('activateQuestionBankBtn')
        .addEventListener('click', async () => {
          ...
          if (!activeRes.ok) return alert(data.message || '切換題庫失敗');
          await loadQuestions();
          await renderEditQuestions();
          alert('已切換學生遊戲使用的題庫');
        });
      ...
      document.getElementById('addQuestionBtn')
        .addEventListener('click', async () => {
          ...
          if (!addRes.ok) {
            ...
            return alert(data.message || '新增題目失敗');
          }
          await loadQuestions();
          renderEditQuestions();
        });
